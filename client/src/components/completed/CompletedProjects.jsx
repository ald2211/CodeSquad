...
const CompletedProjects = () => {
  ...
  useEffect(() => {
    const getCompletedProjects = async () => {
      try {
        let res = await getAllCompletedProjects(
          currentPage,
          itemsPerPage,
          search
        );
  ...
  const handleSubmitReview = async (clientId, developerId, workId, key) => {
    ...
    if (role === "client") {
      ...
      (userId = clientId), (reviewer = developerId);
    }
    let reviewData = {
      userId,
      reviewer,
    ...
    const options = {
      ...
      handler: async function (response) {
        ...
        if (res.data.success) {
          ...
          updatedWorkData[key].paymentId.razorpayPaymentId =
            res.data.paymentData.razorpayPaymentId;
          setCompleted(updatedWorkData);
    ...
    <div className="max-w-6xl mx-auto pt-3 space-y-6 mt-[90px] mb-2">
          ...
          >
            <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <div className="flex justify-between items-center mb-4">
                  ...
                  <div
                    className={`rounded-lg px-3 py-1 text-xs font-bold uppercase text-white ${
              ...
              <div>
                {currentUser.data.role === "developer" ? (
                  <p
                    className={`rounded-lg px-3 py-1 text-xs font-bold uppercase text-white text-center ${
                  ...
                  >
                    Payment {work.paymentId?.developerPayment}
                  </p>
                ) : (
                  <p
                    className={`rounded-lg px-3 py-1 text-xs font-bold uppercase text-white text-center ${
                  ...
                  >
                    Payment {work.paymentId?.clientPayment}
                  </p>
                )}
                <p className="text-gray-600 mt-2">
                  Bid Amount: ₹
                  {
                    work.bids.find((dev) => dev.developer === work.developerId)
                      ?.bidAmount
                  }
                </p>
                  ...
                  <>
                    {work.paymentId.upi && (
                      <p className="text-black mt-2 font-semibold ">
                        your UPI ID is {work.paymentId?.upi}
                      </p>
                    )}
                    <div className="mt-4">
                      <button
                      ...
                      >
                        ...
                        <button
                          ...
                          onClick={() =>
                            handleRequestPayment(work.paymentId, index)
                          }
                        >
                          update UPI
                        </button>
                ...
                {currentUser.data.role === "client" &&
                  (work.paymentId.clientPayment === "pending" ? (
                    ...
                    <PaymentIdCard
                      paymentId={work.paymentId.razorpayPaymentId}
                    />
                  ))}
              </div>
            </div>
            ...
            (currentUser.data.role === "developer" && work.developerReview) ? (
              ...
              <div className="p-6">
                ...
                />
                {ratingError !== "" && (
                  <p className="text-sm text-red-600 mb-[5px]">{ratingError}</p>
                )}
                <button
      ...
      {currentUser.data.role === "client" && (
        <>
          ...
          />
          <PaymentFailed
            isOpen={paymentFailedOpen}
            onClose={() => setPaymentFailedOpen(false)}
          />
        </>
      )}
    </div>
