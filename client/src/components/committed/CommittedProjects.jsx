        ...
        {loading ? (
          <div className="flex items-center justify-center h-screen bg-white">
            <img
              src={spinner}
              alt="Loading"
              className="w-16 h-16 animate-spin mb-32"
            />
          </div>
        ) : committed?.length === 0 ? (
          ...
          committed.map((work, index) => (
            <div
              key={index}
              className="bg-white flex-wrap lg:flex-nowrap md:flex-nowrap  shadow-md rounded-lg overflow-hidden mb-6 p-4 flex "
            >
              ...
              <div className="w-full mt-6">
                    ...
                    <div className="flex items-center mb-3">
                      ...
                      {currentUser?.data?.role === "developer" && (
                        <div className="flex  items-center ml-auto">
                          <button
                            onClick={() => handleVideoCallJoin(work)}
                            className="bg-blue-500 text-white px-4 py-2 rounded-md hover:bg-green-600 focus:outline-none transition duration-200"
                          >
                          ...
                          <button
                            ...
                            className="relative bg-blue-500 ml-2 text-white px-4 py-2 rounded-md hover:bg-green-600 focus:outline-none transition duration-200"
                          >
                            <BsChatSquareDots className="w-8 h-8" />
                            <span
                              className={`absolute left-14 bottom-10  block w-3 h-3 rounded-full ${
                                onlineUsers.includes(work.clientId._id)
                                  ? "bg-green-500"
                                  : "bg-gray-400"
                              }`}
                              title={
                      ...
                      {currentUser?.data?.role === "client" && (
                        <div className="flex  items-center ml-auto">
                          ...
                          <button
                            ...
                            className="bg-blue-500 relative ml-2 text-white px-4 py-2 rounded-md hover:bg-green-600 focus:outline-none transition duration-200"
                          >
                            <BsChatSquareDots className="w-8 h-8" />
                            <span
                              className={`absolute left-14 bottom-10 block w-3 h-3 rounded-full ${
                              ...
                              title={
                                onlineUsers.includes(work.developerId)
                                  ? "Online"
                                  : "Offline"
                              }
                            />
                          ...
                          {work?.projectLink && (
                            <div className="flex items-center space-x-2 p-2 bg-gray-100 border border-gray-300 rounded-md shadow-sm">
                              <span
                                ...
                                onClick={() =>
                                  handleCopyLink(work?.projectLink)
                                }
                                title="Click to copy link"
                              >
                                {work.projectLink.length > 30
                                  ? `${work.projectLink.slice(0, 30)}...`
                                  : work.projectLink}
                              </span>
                            </div>
                          )}
                ...
                {currentUser?.data?.role === "developer" && (
                  <div className="flex items-center w-full bg-blue-100 p-4 rounded-lg shadow-md mt-7">
                    <CiCircleAlert className="text-blue-600 text-2xl mr-3" />
                    <p className="text-blue-800 font-semibold">
                      Once you complete the work, upload the completed project
                      link and inform the admin.
                    </p>
                  </div>
                )}
                <div className="fixed bottom-3 right-1">
                  ...
                  <span
                    className={`absolute bottom-10 right-0 block w-3 h-3 rounded-full ${
                      onlineUsers.includes(adminId)
                        ? "bg-green-500"
                        : "bg-gray-400"
                    }`}
