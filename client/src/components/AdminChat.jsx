  ...
  return (
    <div className="flex flex-row h-screen mt-[80px] overflow-hidden">
      ...
      <div className="flex-1 flex flex-row">
        <div className="w-1/3 bg-white border-r p-1 scroll-m-0 scroll-smooth overflow-y-auto">
  ...
  {userData.map((user) => (
    ...
    >
      <div className="flex items-center">
        ...
        <div className="flex-1 flex flex-col bg-gray-100 h-[480px]">
          {selectedUser ? (
            <>
                ...
                {messages?.length > 0 ? (
                  messages.map((message, index) => (
                    <div key={index} className={`flex ${message.sender === currentUser.data._id ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-xs md:max-w-md lg:max-w-lg p-3 my-2 rounded-lg shadow-md ${message.sender === currentUser.data._id ? 'bg-blue-500 text-white' : 'bg-gray-300 border'}`}>
                        <p>{message.text}</p>
