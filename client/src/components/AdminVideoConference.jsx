...
const AdminVideoConference = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values, actions) => {
      ...
      window.open(`/room/${values.workNumber.toLowerCase()}/${currentUser.data._id}/${values.name}`,'_blank')
      actions.setSubmitting(false);
    },
  });

  const { handleChange, handleBlur, values, errors, handleSubmit, touched, isSubmitting } = formik;
        ...
        <div className="flex-1 p-6 overflow-x-auto">
          <div className="shadow overflow-x-auto border-b border-gray-200 sm:rounded-lg">
            <div className="flex justify-between items-center p-4 bg-white">
              <h1 className="text-2xl font-semibold text-gray-900">Video Conference</h1>
            </div>
            <div className="p-4 bg-white shadow-md rounded-lg">
              <h2 className="text-xl font-medium text-gray-800 mb-4">Join Conference Room</h2>
              <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-6">
                ...
                <div>
                  <label htmlFor="workNumber" className="block text-sm font-medium text-gray-700">Work Number</label>
                  <Input
