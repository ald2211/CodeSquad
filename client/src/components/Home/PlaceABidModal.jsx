...
import {
  ...
  processStart,
  updateWorkSuccess,
} from "../../Redux/user/userSlice"; // Adjust redux actions
    ...
    onSubmit: async (values, actions) => {
      try {
        ...
        const res = await addBid({
          ...values,
          workId,
          developerPhoto: currentUser.data.avatar,
          developerName: currentUser.data.name,
          developerRole: currentUser.data.jobRole,
          developerEmail: currentUser.data.email,
        });
          ...
          <div className="grid grid-cols-1 gap-6 mb-6">
            ...
            <div>
              ...
              {formik.errors.bidAmount && formik.touched.bidAmount && (
                <ShowError Error={formik.errors.bidAmount} />
              )}
            </div>
            <div>
              <label
              ...
              {formik.errors.deliveryTime && formik.touched.deliveryTime && (
                <ShowError Error={formik.errors.deliveryTime} />
              )}
            </div>
          </div>
