...
const BidsModal = ({ bidDetails, handleCloseModal }) => {
  ...
  useEffect(() => {
    const fetchBidDetails = async () => {
      setLoading(true);
      try {
        const res = await getBidDetails(
          bidDetails.workNumber,
          currentPage,
          bidsPerPage
        );
    ...
    >
      <div className="bg-white p-6 rounded-lg shadow-lg w-full h-full max-w-6xl max-h-[75vh] mt-[95px] overflow-auto">
        ...
        <p className="text-gray-600 mb-4">
          Bid End Date: {bidDetails.bidEndDate.split("T")[0]}
        </p>
        {loading ? (
            ...
            {bids.length === 0 ? (
              ...
              <>
                    ...
                    <tbody>
                      {bids.map((bid, index) => (
                        <tr key={index}>
                          ...
                          <td
                            onClick={() => handleViewMore(bid.developer)}
                            ...
                            <td
                              onClick={() =>
                                handleBidAccept({
                                  ...
                                  devName: bid.developerName,
                                })
                              }
                              className="py-2 px-4 border-b text-center rounded border-[1px] hover:bg-blue-500 hover:text-white cursor-pointer"
                            >
                              Accept
                            </td>
                          )}
                ...
                <div className="flex justify-center mt-4 space-x-1">
                  ...
                  {[...Array(Math.ceil(totalBids / bidsPerPage)).keys()].map(
                    (pageNumber) => (
                      <button
                        key={pageNumber}
                  ...
                  <button
                    className={`px-4 py-2 border rounded-r-lg ${
                      currentPage === Math.ceil(totalBids / bidsPerPage)
                        ? "opacity-50 cursor-not-allowed"
                        : "hover:bg-gray-200"
                    }`}
                    onClick={() => paginate(currentPage + 1)}
