...
class workRepository {
    ...
    const query = search
      ? {
          $or: [
            { workName: new RegExp(search, "i") },
            { workType: new RegExp(search, "i") },
            { workStatus: new RegExp(search, "i") },
          ],
        }
      : {};
  ...
  async findWorkByDeveloperId(id, role) {
    ...
    return stats.length > 0
      ? stats[0]
      : { committedProjects: 0, completedProjects: 0 };
  }
  ...
  async createBid(id, bidDetails, completedWorks) {
    ...
    return await Work.updateOne(
      { workNumber: bidDetails.workId },
      { $push: { bids: data } }
    );
  }
