...
class workSerivice {
  ...
  async getClientWorks(
    ...
    return await workRepository.findAllWorksByUserId(
      role,
      clientId,
      page,
      limit,
  ...
  async updateWork(role, workId, workData, clientId) {
    ...
    const updatedUser = await workRepository.findByWorkIdAndUpdate(
      workId,
      updateWorkData
    );
  ...
  async deleteClientWorkByWorkId(role, workNumber, clientId) {
    const workToRemove = await workRepository.deleteWorkByWorkNumber(
      workNumber
    );
  ...
  async placeBid(role, Id, bidDetails) {
    ...
    if (work.budget < bidDetails.bidAmount)
      throw errorHandler(400, "bid amount must be less than budget");
    if (work.expectedDelivery < bidDetails.deliveryTime)
      throw errorHandler(
        400,
        `project should be delivered in ${work.expectedDelivery} days`
      );

    const updatedData = await workRepository.createBid(
      Id,
      bidDetails,
      completedWorks
    );
  ...
  async acceptDeveloperBid(role, Id, work) {
    ...
    const updatedData = await workRepository.findByWorkIdAndUpdate(
      work.workNumber,
      updateWorkData
    );
