...
class authService {
  async userSignUp({ name, email, password, username, role }) {
    if (
      ...
      throw errorHandler(400, "Invalid email address");

    if (!password || password.length < 8)
      throw errorHandler(400, "Password must be at least 8 characters long");

    if (!username || username.trim().length < 2)
      throw errorHandler(400, "Name must be at least 2 characters long");
  ...
  async userSignIn({ email, password }) {
    ...
    if (!validPassword) throw errorHandler(401, "Invalid email or password");
  ...
  async handleForgotPassword(email) {
    ...
    const token = jwt.sign({ id: user[0]._id }, process.env.JWT_RESET_SECRET, {
      expiresIn: "10m",
    });
  ...
  async handleResetPassword(token, newPassword) {
    ...
    if (!decoded)
      throw errorHandler(400, "token verification failed, please try again");
    const hashedPassword = await bcrypt.hash(newPassword, 10);

    const result = await userRepository.findByIdAndUpdate(decoded.id, {
