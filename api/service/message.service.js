...
class messageService {
  ...
  async getConversation(senderId, receiverId) {
    const userConversation = await conversationRepository.findMessages(
      senderId,
      receiverId
    );
    ...
    if (userConversation) {
      const messageIds = userConversation.messages.map(
        (message) => message._id
      );
  ...
  async markRead(id, userId) {
    ...
    const userConversation = await conversationRepository.findByMessageId(
      message._id
    );
