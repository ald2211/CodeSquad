...
export const signin = async (req, res, next) => {
  try {
    const { data, accessToken, refreshToken } = await authService.userSignIn(
      req.body
    );
    res
...
export const OAuthSignin = async (req, res, next) => {
  try {
    const { data, accessToken, refreshToken } =
      await authService.userOAuthSignup(req.body);
    res
      .cookie("refresh_token", refreshToken, {
        httpOnly: true,
        expires: new Date(Date.now() + 24 * 60 * 60 * 1000), // 24 hours
...
export const resetPassword = async (req, res, next) => {
  try {
    ...
    if (err.name === "TokenExpiredError")
      err.message = "Token has expired, please request a new reset link";
    next(err);
  }
};
