...
export const addProjects = async (req, res, next) => {
  ...
  try {
    const userProjects = await projectService.addUserProject(
      req.user.id,
      req.body
    );
    res.status(200).json({
...
export const editProjects = async (req, res, next) => {
  try {
    const userProjects = await projectService.editUserProject(
      req.params.user_id,
      req.params.proj_id,
      req.body
    );
...
export const deleteProjects = async (req, res, next) => {
  try {
    const userProjects = await projectService.deleteUserProject(
      req.user.id,
      req.params.id
    );
    res.status(200).json({
