...
export const getAllUsers = async (req, res, next) => {
  try {
    ...
    const { count, users } = await adminService.findAllUsers(
      page,
      limit,
      search
    );
...
export const updateProjectState = async (req, res, next) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: "Project Status updated successfully" });
  } catch (err) {
    next(err);
  }
};
...
export const findAllUserWorks = async (req, res, next) => {
  try {
    ...
    const limit = parseInt(req.query.limit) || 10;
    const search = req.query.search || "";
    const { count, data } = await adminService.findAllWorks(
      req.user.role,
      page,
      limit,
      search
...
export const findAllCompletedWorks = async (req, res, next) => {
  try {
    ...
    const limit = parseInt(req.query.limit) || 10;
    const search = req.query.search || "";
    const { count, data } = await adminService.findCompletedWorks(
      req.user.role,
      page,
      limit,
      search
