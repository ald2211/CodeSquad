...
export const getEducation = async (req, res, next) => {
  try {
    const userEducation = await educationService.getAllUserEducation(
      req.user.id
    );
    res.status(200).json({
      success: true,
...
export const addEducation = async (req, res, next) => {
  ...
  try {
    ...
    const userEducation = await educationService.addUserEducation(
      req.user.id,
      req.body
    );
    res.status(200).json({
...
export const editEducation = async (req, res, next) => {
  try {
    const userEducation = await educationService.editUserEducation(
      req.params.user_id,
      req.params.edu_id,
      req.body
    );
...
export const deleteEducation = async (req, res, next) => {
  try {
    const userEducation = await educationService.deleteUserEducation(
      req.user.id,
      req.params.id
    );
    res.status(200).json({
